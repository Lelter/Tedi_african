...
data=pd.read_excel("predict.xlsx",index_col=0)
print(data)
dta=list(data["销售额"])
print(dta)
...
PredicValue=[]
PredicValue.append(train.values[-1])
for i in range(len(forecast)):
    PredicValue.append(forecast[i])
PredicValue=pd.Series(PredicValue)

PredicValue.index = pd.Index(sm.tsa.datetools.dates_from_range('2019Q4',length=8))
...
ax = dta.loc['2017':].plot(ax=ax,label='训练值')
